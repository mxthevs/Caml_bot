...
type t = { prefix : string option; command : command }
...
let split_params params =
  ...
  match trail with None -> tokens | Some trail -> tokens @ [ trail ]

let fail_ msg err = raise (ParseError (msg, err))

(* expect exactly one word *)
let expect1 msg = function [ x ] -> x | _ -> fail_ msg "expected one parameter"

and expect2 msg = function [ x; y ] -> (x, y) | _ -> fail_ msg "expected two parameters"
...
let parse_exn msg =
  if String.length msg = 0 then fail_ msg "Zero-length message"
  else
    ...
    let command =
      ...
      | "PRIVMSG" ->
          ...
          let user = match prefix with Some s -> extract_user_from_prefix s | None -> "" in
